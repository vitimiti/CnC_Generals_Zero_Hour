...
#include "NewSAGE/Compression/Compression.h"
#include "NewSAGE/Compression/LZHCompress/NOXCompress.h"
...
#include "NewSAGE/Compression/EAC/RefCodex.h"
// clang-format on
...
const char* CompressionManager::getCompressionNameByType(CompressionType compType) {
    static char const* s_compressionNames[COMPRESSION_MAX + 1] = {
...
char const* CompressionManager::getDecompressionNameByType(CompressionType compType) {
    static char const* s_decompressionNames[COMPRESSION_MAX + 1] = {
        "d_None", "d_RefPack",
        /*
        "d_NoxLZW",
        "d_ZLib1",
...
Bool CompressionManager::isDataCompressed(void const* mem, Int len) {
    CompressionType t = getCompressionType(mem, len);
    return t != COMPRESSION_NONE;
}

CompressionType CompressionManager::getPreferredCompression(void) { return COMPRESSION_REFPACK; }

CompressionType CompressionManager::getCompressionType(void const* mem, Int len) {
    if (len < 8)
...
Int CompressionManager::getUncompressedSize(void const* mem, Int len) {
    if (len < 8)
        return len;
