...
static void BTREE_writebits(struct BTreeEncodeContext* EC, struct BTREEMemStruct* dest, unsigned int bitpattern,
                            unsigned int len) {
    if (len > 16) {
...
static void BTREE_joinnodes(struct BTreeEncodeContext* EC, unsigned char* cluep, unsigned char* rightp,
                            unsigned char* joinp, unsigned int clue) {
    unsigned char* s;
    unsigned char* d;
    unsigned char* bend;
    unsigned int   i;
...
static unsigned int BTREE_findbest(BTREEWORD* countptr, unsigned char* tryq, unsigned int* bestn, unsigned int* bestval,
                                   int ratio) {
    unsigned int i;
    unsigned int i1;
    unsigned int i2;
...
static void BTREE_treepack(struct BTreeEncodeContext* EC, struct BTREEMemStruct* dest, unsigned int passes,
                           unsigned int multimax, unsigned int quick, int zerosuppress) {
...
static int BTREE_compressfile(struct BTreeEncodeContext* EC, struct BTREEMemStruct* infile,
                              struct BTREEMemStruct* outfile, int ulen, int zerosuppress) {
    unsigned int i;
...
int GCALL BTREE_encode(void* compresseddata, void const* source, int sourcesize, int* opts) {
    int                       plen;
