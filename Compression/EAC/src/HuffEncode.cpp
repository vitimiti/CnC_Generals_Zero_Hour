...
static void HUFF_deltabytes(void const* source, void* dest, int len) {
...
static void HUFF_writebits(struct HuffEncodeContext* EC, struct HUFFMemStruct* dest, unsigned int bitpattern,
                           unsigned int len) {
    if (len > 16) {
...
static void HUFF_treechase(struct HuffEncodeContext* EC, unsigned int node, unsigned int bits) {
    if (node < HUFFCODES)
...
static int HUFF_minrep(struct HuffEncodeContext* EC, unsigned int remaining, unsigned int r) {
...
static void HUFF_writenum(struct HuffEncodeContext* EC, struct HUFFMemStruct* dest, unsigned int num) {
    unsigned int dphuf;
    unsigned int dbase;
...
static void HUFF_writeexp(struct HuffEncodeContext* EC, struct HUFFMemStruct* dest, unsigned int code) {
...
static void HUFF_writecode(struct HuffEncodeContext* EC, struct HUFFMemStruct* dest, unsigned int code) {
    if (code == EC->clue)
...
static void HUFF_pack(struct HuffEncodeContext* EC, struct HUFFMemStruct* dest, unsigned int opt) {
    unsigned char* bptr1;
    unsigned char* bptr2;
    unsigned int   i;
    unsigned int   i1;
    ...
    */
    ...
    while (bptr1 < EC->bufptr) {
        ...
        if (((std::intptr_t)bptr1 - (std::intptr_t)EC->buffer) >= (std::intptr_t)(EC->plen + curpc))
            curpc = (std::intptr_t)bptr1 - (std::intptr_t)EC->buffer - EC->plen;
    }
...
static int HUFF_packfile(struct HuffEncodeContext* EC, struct HUFFMemStruct* infile, struct HUFFMemStruct* outfile,
                         int ulen, int deltaed) {
    unsigned int i;
...
int GCALL HUFF_encode(void* compresseddata, void const* source, int sourcesize, int* opts) {
    int                       plen = 0;
