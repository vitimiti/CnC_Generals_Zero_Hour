...
#define HASH(cptr)                                                                                                     \
    (int)((((unsigned int)(unsigned char)cptr[0] << 8) | ((unsigned int)(unsigned char)cptr[2])) ^                     \
...
int GCALL REF_encode(void* compresseddata, void const* source, int sourcesize, int* opts) {
    int maxback = 131072;
    int quick   = 0;
    int plen;
    int hlen;
    ...
    plen =
        hlen + refcompress((unsigned char*)source, sourcesize, (unsigned char*)compresseddata + hlen, maxback, quick);
    return (plen);
}
