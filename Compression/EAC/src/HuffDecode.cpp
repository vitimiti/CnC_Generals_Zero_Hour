...
bool GCALL HUFF_is(void const* compresseddata) {
    bool ok       = false;
    int  packtype = ggetm(compresseddata, 2);

    if (packtype == 0x30fb || packtype == 0x31fb || packtype == 0x32fb || packtype == 0x33fb || packtype == 0x34fb ||
        packtype == 0x35fb || packtype == 0xb0fb || packtype == 0xb1fb || packtype == 0xb2fb || packtype == 0xb3fb ||
        packtype == 0xb4fb || packtype == 0xb5fb)
        ok = true;
...
int GCALL HUFF_size(void const* compresseddata) {
    int len      = 0;
    int packtype = ggetm(compresseddata, 2);
...
int GCALL HUFF_decode(void* dest, void const* compresseddata, int* compressedsize) {
